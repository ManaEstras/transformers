...
from ...processing_utils import ProcessorMixin, MultiModalData, ImagesKwargs, VideosKwargs, ProcessingKwargs
...
class HunYuanVLVideosProcessorKwargs(VideosKwargs, total=False):
...
class HunYuanVLProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor=None, tokenizer=None, video_processor=None, chat_template=None, **kwargs):
        ...
        for attr in required_attrs:
            if not hasattr(tokenizer, attr):
                raise ValueError(f"Tokenizer missing required attribute '{attr}'. "
                    f"Please add the corresponding field mapping to `extra_special_tokens` in tokenizer_config.json")
        ...
        self.video_start_token = "<｜hy_place▁holder▁no▁104｜>" if not hasattr(tokenizer, "video_start_token") else tokenizer.video_start_token
        ...
        self.video_end_token = "<｜hy_place▁holder▁no▁105｜>" if not hasattr(tokenizer, "video_end_token") else tokenizer.video_end_token
        ...
        # HyV1 appends image_begin/image_end tokens around each image (+2 tokens).
        # HyV3 sets cat_extra_token=0 in vision_config, so its subclass overrides
        # this to False.
    ...
    def __call__(
        ...
        **kwargs
        ...
        if images is not None:
            ...
            for i in range(len(text)):
                while self.image_token in text[i]:
                    ...
                    # text[i] = text[i].replace(self.image_token, self.image_start_token + self.placeholder_token * num_image_tokens + self.image_end_token, 1)
        ...
        if videos is not None:
            ...
            for i in range(len(text)):
                while self.video_token in text[i]:
                    ...
                    grid_t = (grid_t + self.image_processor.temporal_patch_size - 1) // self.image_processor.temporal_patch_size
                    patch_h = grid_h // self.image_processor.merge_size
                    patch_w = grid_w // self.image_processor.merge_size
                    ...
                    for _ in range(grid_t):
                        replace_str += self.image_start_token + self.placeholder_token * num_image_tokens + self.image_end_token
        ...
        if images is not None or videos is not None:
            image_start_token_pos_indices = torch.where(input_ids[0] == self.image_start_token_id)[0]
            image_count = 0
            video_index = -1
            image_index = -1
            video_mode = False
            while image_count < len(image_start_token_pos_indices):
                ...
                if prev_token == self.video_start_token_id:
                    ...
                    for image_count_new in range(image_count, image_count + grid_t):
                        ...
                        position_ids_t[start_pos: start_pos + replace_num] = image_count_new
                    image_count += grid_t
                else:
                    image_index += 1
    ...
    def _get_num_multimodal_tokens(self, image_sizes=None, video_sizes=None, **kwargs):
        ...
        if image_sizes is not None:
            merge_size = kwargs.get("merge_size", None) or self.image_processor.merge_size
            ...
            num_image_tokens = [
                patch_hw[0] // merge_size * (patch_hw[1] // merge_size + 1)
                + (2 if self.cat_extra_token else 0)
