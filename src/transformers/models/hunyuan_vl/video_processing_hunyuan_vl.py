"""video processor class for HunYuan-VL."""
...
from .image_processing_hunyuan_vl import smart_resize


class HunYuanVLVideoProcessorInitKwargs(VideosKwargs):
...
class HunYuanVLVideoProcessor(BaseVideoProcessor):
    ...
    valid_kwargs = HunYuanVLVideoProcessorInitKwargs
    model_input_names = ["pixel_values_videos", "video_grid_thw"]

    def __init__(self, **kwargs: Unpack[HunYuanVLVideoProcessorInitKwargs]):
        ...
        self.resample = PILImageResampling.BICUBIC
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            # Note: HunYuanVL Video model uses Conv2d (not Conv3d), so we process frames independently
            # temporal_patch_size is NOT used for merging here - each frame is a separate image
            patches = patches.view(
                batch_size,
                grid_t,
                1,  # Each frame processed independently (no temporal merging)
                channel,
                grid_h // merge_size,
                merge_size,
                patch_size,
            ...
            flatten_patches = patches.reshape(
                ...
                channel * 1 * patch_size * patch_size,  # 3 * 16 * 16 = 768 per patch
            )
...
__all__ = ["HunYuanVLVideoProcessor"]
